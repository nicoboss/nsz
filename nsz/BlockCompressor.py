...
def blockCompress(filePath, compressionLevel, noIntro, useLongDistanceMode, blockSizeExponent, outputDir, threads):
	if filePath.suffix == '.nsp':
		return blockCompressNsp(filePath, compressionLevel, noIntro, useLongDistanceMode, blockSizeExponent, outputDir, threads)
	elif filePath.suffix == '.xci':
		return blockCompressXci(filePath, compressionLevel, noIntro, useLongDistanceMode, blockSizeExponent, outputDir, threads)

def blockCompressContainer(readContainer, writeContainer, compressionLevel, noIntro, useLongDistanceMode, blockSizeExponent, threads):
	CHUNK_SZ = 0x100000
...
def blockCompressNsp(filePath, compressionLevel, noIntro, useLongDistanceMode, blockSizeExponent, outputDir, threads):
...
def blockCompressXci(filePath, compressionLevel, noIntro, useLongDistanceMode, blockSizeExponent, outputDir, threads):
	filePath = filePath.resolve()
	container = factory(filePath)
	container.open(str(filePath), 'rb')
	...
	try:
		with Xci.XciStream(str(xczPath), originalXciPath = filePath) as xci: # need filepath to copy XCI container settings
			with Hfs0.Hfs0Stream(xci.hfs0.add('secure', 0), xci.f.tell()) as secureOut:
				blockCompressContainer(secureIn, secureOut, compressionLevel, noIntro, useLongDistanceMode, blockSizeExponent, threads)
